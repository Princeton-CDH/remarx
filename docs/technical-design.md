...
- **Quote Consolidation.** In this optional step, detected sentence pairs that are sequential in both original and reuse corpora are consolidated into multi-sentence passages. To aid analysis, the consolidated quotes includes an additional field indicating the number of sentences included.

### Sentence Corpus Builder

This component is a python program that extracts sentences from an input text and generates a corresponding
sentence-level corpus CSV file. It consists of the following:

- text extraction
- sentence segmentation
- filtering (exclude two-word and numeric & punctuation-only "sentences")
...
Any fields included with the input sentences will be passed through and prefixed based on the input corpus (either `reuse_` or `original_`).
