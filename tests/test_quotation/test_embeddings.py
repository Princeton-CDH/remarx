...
import remarx.quotation.embeddings
...
def test_sentence_transformers_import_error():
    ...
    with (
        ...
        pytest.raises(ImportError, match="sentence-transformers library is required"),
    ):
        # Force reimport to trigger the ImportError
        importlib.reload(remarx.quotation.embeddings)
